...
class _GetComplainState extends State<GetComplain> {
  Future<List<complainModel>> mygetcomplain()async{
    final response = await http.get(Uri.parse('https://unmusical-boil.000webhostapp.com/getcomplain.php'));
...
Future<void> mydeletecomplain(String id)async{
  final response = await http.post(Uri.parse('https://unmusical-boil.000webhostapp.com/deletecomplain.php'),body: {'id':id});
  if(response.statusCode==200){
...
@override
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Complain Data')),
          ...
          return ListView.builder(itemCount: mylist.length,itemBuilder: (context,index){
            return Card(child: Column(children: [
              Text('id = ${mylist[index].id}'),
               Text('Topic = ${mylist[index].topic}'),
                Text('Description = ${mylist[index].description}'),
                 Text('Date = ${mylist[index].date}'),
                 Row(children: [IconButton(onPressed: (){
                         setState(() {
                           mydeletecomplain(mylist[index].id!);
                           Navigator.push(context, MaterialPageRoute(builder: (context)=>GetComplain()));
                         });
